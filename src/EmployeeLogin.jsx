...
export default function EmployeeLogin({ onAdminLogin, externalNotice = "" }) {
  const [notice, setNotice] = useState(null);
  const authMethod = (import.meta.env.VITE_AUTH_METHOD || "google").toLowerCase();
  const oidcProvider = import.meta.env.VITE_OIDC_PROVIDER || "";
  const oidcButtonLabel =
    import.meta.env.VITE_OIDC_BUTTON_LABEL || "Continue with Company Login";

  async function handleLogin() {
    const redirectTo = `${window.location.origin}/`;
    const provider = authMethod === "oidc" ? oidcProvider : "google";

    if (authMethod === "oidc" && !oidcProvider) {
      setNotice({
        tone: "error",
        message: "Missing VITE_OIDC_PROVIDER in .env",
      });
      return;
    }
    ...
    if (error) {
      setNotice({
        tone: "error",
        message:
          (authMethod === "oidc" ? "Company sign-in failed: " : "Google sign-in failed: ") +
          error.message,
      });
    }
  }
  ...
  return (
    <div className="min-h-screen bg-[radial-gradient(circle_at_top_left,_rgba(59,130,246,0.11),_transparent_30%),radial-gradient(circle_at_top_right,_rgba(16,185,129,0.08),_transparent_25%),linear-gradient(to_bottom,#f9fafb,#f4f6fb)] px-4 py-12 text-zinc-900 sm:px-6 lg:px-8">
      ...
      <div className="mx-auto max-w-md rounded-[30px] border border-zinc-200/80 bg-white/95 p-7 shadow-[0_20px_55px_rgba(15,23,42,0.12)] ring-1 ring-white sm:p-8">
        ...
        >
          {authMethod === "oidc" ? oidcButtonLabel : "Continue with Google"}
        </button>
